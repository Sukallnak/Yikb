...
export interface Entity {
  ...
  // C-Moon Punishment visual/control state
  cmoonFloatUntil?: number;
  cmoonFloatStart?: number;
  cmoonFloatSourceY?: number;
  // AI anti-stuck
  stuckAcc?: number;
  lastPos?: Vec2;
}
...
export interface Projectile {
  ...
  applyElectro?: number;
  // applies a direct stun on hit (Whitesnake Hypnotic Spit)
  stunSeconds?: number;
  ...
  hurtsPlayer?: boolean;
  // Optional projectile knockback on hit (Whitesnake final Improvisation bullet).
...
export interface ItemPickup {
  id: number;
  kind: "arrow" | "disc" | "requiem_arrow" | "blue_pebble" | "strange_hat" | "green_arrow" | "green_baby";
  pos: Vec2;
  bornAt: number;
}
