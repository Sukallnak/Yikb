...
const ENEMY_ATTACK_RANGE = 32; // parity with player basic punch (range 18 + radius 14)
...
const PROP_BREAKERS_BY_STAND = new Set<StandId>(["star_platinum", "sptw", "c_moon"]);
...
interface World {
  ...
  curseStrikes: { targetId?: number; targetPos?: Vec2; hitAt: number; dmg: number; color: string; radius?: number; stun?: number; kind?: string }[];
  // Harvest runtime
  ...
  primeTickAt?: number;
  primeIndex?: number;
  primeHealUntil?: number;
  primeHealCooldownUntil?: number;
  greenArrowCount?: number;
  greenBabyCount?: number;
  cmoonGravityUntil?: number;
  cmoonGravityStartedAt?: number;
  cmoonGravityRadius?: number;
  cmoonGravityAffected?: number[];
  // Scheduled projectile spawns (Whitesnake Improvisation 3-burst rhythm, etc.)
  pendingShots?: { fireAt: number; pos: Vec2; dir: Vec2; speed: number; range: number; damage: number; radius: number; color: string; knockback?: number; textGlyph?: string; sourceAbilityKey?: "m1" | "a1" | "a2" | "a3" | "a4" }[];
  // Stands the player has ever equipped — drives "???" in the Tonth catalog.
  standsUnlocked?: StandId[];
  // Combat noise window — neutral NPCs near the player will flee while > w.time.
  combatNoiseUntil?: number;
...
function damageEntity(w: World, e: Entity, dmg: number, knockback?: { dir: Vec2; amount: number }, crit = false, opts?: { fromPuppet?: boolean }) {
  ...
  // C-Moon inversion passive: chance to reverse incoming damage back into the attacker.
  if (e.kind === "player" && w.standId === "c_moon" && dmg > 0 && Math.random() < 0.127) {
    const attacker = nearestTarget(w, w.player.pos, 70);
    if (attacker) {
      spawnVfx(w, { kind: "shockwave", pos: { ...w.player.pos }, radius: 26, color: "#a06bff", life: 0.35 });
      spawnVfx(w, { kind: "beam", pos: { ...w.player.pos }, to: { ...attacker.pos }, color: "#d6c5ff", life: 0.22 });
      damageEntity(w, attacker, dmg);
      showToast(w, "Inversion reflected");
      return;
    }
  }
  ...
  if (dmg > 0) {
    const bloodColor = e.kind === "player" ? "#8f0f18" : e.kind === "enemy" ? "#b21717" : "#7d1020";
    spawnParticles(w, { x: e.pos.x, y: e.pos.y - 3 }, bloodColor, crit ? 10 : 6, {
      shape: "circle", gravity: 110, speedMin: 30, speedMax: crit ? 160 : 95, life: 0.55,
    });
    spawnVfx(w, { kind: "slash_hit", pos: { ...e.pos }, radius: crit ? 16 : 10, color: bloodColor, life: 0.22 });
  }
  if (crit) {
...
function m1DamageRoll(w: World, puppetSwing: boolean): { dmg: number; crit: boolean } {
  ...
  if (sid === "whitesnake")     return { dmg: crit ? 4.2 : 2.6, crit };
  if (sid === "c_moon")         return { dmg: crit ? 5.5 : 3.5, crit };
...
function castAbility(w: World, key: "m1" | "a1" | "a2" | "a3" | "a4", input: InputState) {
  ...
  // Stand desummoned: only M1 works (player throws their own punch with the stand's M1 stats x0.6)
  if (stand.id !== "none" && !w.standActive && key !== "m1") {
    softBanner(w, "stand_off", "Resummon stand to use abilities", 0.8);
    return;
  }
  // C-Moon Punishment freezes the player's M1 while the gravity bubble is up.
  if (key === "m1" && w.standId === "c_moon" && (w.cmoonGravityUntil ?? 0) > w.time) {
    return;
  }
  ...
  const ZERO_DAMAGE_ALLOWED = new Set<string>([
    ...
    "ph_pilot_toggle", "cleansly_violence", "explosion_text", "ice_heal", "lunar_veil", "prime_heal",
    "harvest_gather", "harvest_carry", "sptw_rage", "cmoon_punishment",
  ]);
  ...
  const SELF_OR_UTILITY = new Set<string>([
    ...
    "shard_teleport", "frog_summon", "cleansly_violence", "lunar_veil", "sptw_rage", "prime_heal",
  ...
  switch (ab.kind) {
    case "melee": {
      ...
      // M1 without stand summoned: player swings their own fist, 60% damage.
      const playerOwnPunch = key === "m1" && w.standId !== "none" && !w.standActive;
      if (playerOwnPunch) dmg *= 0.6;
    ...
    // ---- RHCP A3 — Lightning Pillar: telegraph ring → vertical electric column ----
    case "lightning_pillar": {
      const target = nearestTarget(w, p, ab.range);
      const tx = target ? target.pos.x : p.x + dir.x * ab.range;
      const ty = target ? target.pos.y : p.y + dir.y * ab.range;
      const tpos = { x: tx, y: ty };
      // 0.4s telegraph ring
      spawnVfx(w, { kind: "shockwave", pos: tpos, radius: ab.radius ?? 28, color: "#fff36b", life: 0.4 });
      // 0.35s after telegraph, strike
      w.curseStrikes.push({
        targetPos: tpos, hitAt: w.time + 0.4, dmg: ab.damage,
        radius: ab.radius ?? 28, stun: 0.6, color: ab.color,
    ...
    case "ws_improvisation": {
      // Glock-paced 3-burst: fire one bullet now and schedule two more at 1.5s + 3.0s.
      // Slower travel so each round is actually readable, last bullet carries knockback.
      const speed = 220;
      const baseShot = {
        pos: { x: p.x, y: p.y },
        dir: { x: dir.x, y: dir.y },
      ...
      w.pendingShots = w.pendingShots ?? [];
      w.pendingShots.push({ ...baseShot, fireAt: w.time });
      w.pendingShots.push({ ...baseShot, fireAt: w.time + 1.5 });
      w.pendingShots.push({ ...baseShot, fireAt: w.time + 3.0, knockback: ab.knockback ?? 120, color: "#ffffff", radius: (ab.radius ?? 4) + 1 });
      spawnVfx(w, { kind: "stab_line", pos: { ...p }, to: { x: p.x + dir.x * 30, y: p.y + dir.y * 30 }, color: ab.color, life: 0.18 });
      spawnParticles(w, { x: p.x + dir.x * 10, y: p.y + dir.y * 10 }, "#eef2ff", 8, { speedMin: 60, speedMax: 180, life: 0.25 });
      break;
    }
    case "ws_disc_steal": {
      const t = nearestTarget(w, p, ab.range + 30);
      if (!t) break;
      damageEntity(w, t, ab.damage);
      t.slowUntil = w.time + (ab.stunSeconds ?? 6.3);
      // implant DoT (steamy bubbly)
      t.poisonUntil = w.time + (ab.duration ?? 10);
      t.poisonNextTickAt = w.time + 2;
      t.poisonDps = 3.4;
      spawnVfx(w, { kind: "poison_cloud", pos: { ...t.pos }, color: ab.color, radius: 18, life: 0.6 });
      break;
    }
    case "ws_acidic_stab": {
      const t = nearestTarget(w, p, ab.range + 20);
      if (!t) break;
      damageEntity(w, t, ab.damage);
      t.poisonUntil = w.time + (ab.duration ?? 7);
      t.poisonNextTickAt = w.time + 1;
      t.poisonDps = 1;
      spawnVfx(w, { kind: "slash_hit", pos: { ...t.pos }, color: ab.color, radius: 14, life: 0.3 });
      break;
    }
    case "ws_hypnotic_spit": {
      w.projectiles.push({
        id: w.nextId++,
        pos: { x: p.x + dir.x * 14, y: p.y + dir.y * 14 },
        vel: { x: dir.x * (ab.speed ?? 280), y: dir.y * (ab.speed ?? 280) },
        radius: ab.radius ?? 8,
        damage: 0,
        color: ab.color,
        ownerKind: "player",
        ...
        sourceStandId: w.standId,
        sourceAbilityKey: key,
        stunSeconds: ab.stunSeconds ?? 12,
      });
      break;
    }
    case "prime_heal": {
      if (w.standId !== "whitesnake" && w.standId !== "c_moon") break;
      // Hard 20s cooldown gate (regardless of cast time) + skip if already full HP.
      if ((w.primeHealCooldownUntil ?? 0) > w.time) {
        softBanner(w, "prime_cd", `Prime heal ${Math.ceil((w.primeHealCooldownUntil ?? 0) - w.time)}s`, 0.8);
        return;
      }
      if (w.player.hp >= w.player.maxHp) {
        softBanner(w, "prime_full", "Already at full HP", 0.6);
        return;
      }
      if ((w.primeHealUntil ?? 0) > w.time) return;
      w.primeIndex = 0;
      w.primeTickAt = w.time;
      w.primeHealUntil = w.time + (ab.duration ?? 14.4) + 0.25;
      w.primeHealCooldownUntil = w.time + 20;
      w.bannerText = "Prime heal";
      w.bannerUntil = w.time + 1.0;
      spawnVfx(w, { kind: "shockwave", pos: { ...w.player.pos }, radius: 38, color: ab.color, life: 0.45 });
      spawnParticles(w, w.player.pos, ab.color, 22, { shape: "spark", gravity: -80, speedMin: 20, speedMax: 120, life: 0.8 });
      break;
    }
    case "cmoon_punishment": {
      // Centered on the player. Lower radius; distance falloff (4–9% per ring).
      const cx = w.player.pos.x, cy = w.player.pos.y;
      const radius = ab.radius ?? 60;
      const dur = ab.duration ?? 6;
      w.cmoonGravityUntil = w.time + dur;
      w.cmoonGravityStartedAt = w.time;
      w.cmoonGravityRadius = radius;
      w.cmoonGravityAffected = [];
      w.player.stunUntil = Math.max(w.player.stunUntil, w.time + dur);
      for (const e of w.npcs) {
        if (!e.alive) continue;
        const d2v = dist2(e.pos, { x: cx, y: cy });
        if (d2v < radius * radius) {
          const d = Math.sqrt(d2v);
          const t = d / radius; // 0 (center) → 1 (edge)
          // 4% per quarter-radius beyond center, capped at ~36% reduction at the very edge.
          const falloff = Math.max(0.55, 1 - t * 0.45);
          const dmg = ab.damage * falloff;
          w.cmoonGravityAffected.push(e.id);
          e.stunUntil = w.time + dur;
          e.slowUntil = w.time + dur;
          e.cmoonFloatUntil = w.time + dur;
          e.cmoonFloatStart = w.time;
          e.cmoonFloatSourceY = e.pos.y;
          w.curseStrikes.push({ targetId: e.id, hitAt: w.time + dur, dmg, color: ab.color, kind: "cmoon_drop" });
        }
      }
      spawnVfx(w, { kind: "shockwave", pos: { x: cx, y: cy }, radius, color: ab.color, life: 0.6 });
      spawnVfx(w, { kind: "tree_aura", pos: { x: cx, y: cy }, radius, color: ab.color, life: dur });
      w.bannerText = "PUNISHMENT";
      w.bannerUntil = w.time + 1.2;
      break;
    }
    case "cmoon_shotgun": {
      // 4–5 debris shards rise from the ground and fly toward the nearest target.
      const t = nearestTarget(w, p, ab.range + 40);
      if (!t) break;
      const count = 4 + (Math.random() < 0.5 ? 1 : 0);
      const baseDir = norm({ x: t.pos.x - p.x, y: t.pos.y - p.y });
      const speed = ab.speed ?? 360;
      for (let i = 0; i < count; i++) {
        const spread = (i - (count - 1) / 2) * 0.14;
        const cs = Math.cos(spread), sn = Math.sin(spread);
        const dx = baseDir.x * cs - baseDir.y * sn;
        const dy = baseDir.x * sn + baseDir.y * cs;
        w.projectiles.push({
          id: w.nextId++,
          pos: { x: p.x + dx * 10, y: p.y + dy * 10 + 2 },
          ...
          pierce: false,
          hitSet: new Set(),
          expireAt: w.time + ab.range / speed,
          sourceStandId: w.standId,
          sourceAbilityKey: key,
          textGlyph: "DEBRIS",
        });
        spawnParticles(w, { x: p.x + dx * 6, y: p.y + dy * 6 }, "#6b5a40", 4, { gravity: 60, life: 0.4 });
      }
      spawnVfx(w, { kind: "shockwave", pos: { ...p }, radius: 22, color: "#9b8c6b", life: 0.3 });
      break;
    }
  }
  // Anything we cast counts as combat noise — neutral NPCs near the player will flee.
  w.combatNoiseUntil = Math.max(w.combatNoiseUntil ?? 0, w.time + 2.5);
...
export function update(w: World, input: InputState, dt: number) {
  ...
  if (pl.alive) {
    ...
    w.lastJoyMag = len;
    const gravityLocked = w.standId === "c_moon" && !!w.cmoonGravityUntil && w.time < w.cmoonGravityUntil;
    if (!gravityLocked && (!piloting || (w.standId === "harvest" && w.harvestCarryActive && w.standActive)) && len > 0.05) {
  ...
  if (pl.alive && w.m1Held && w.cdTimers.m1 <= 0) {
    castAbility(w, "m1", input);
  }
  if (w.primeHealCooldownUntil && w.time >= w.primeHealCooldownUntil) w.primeHealCooldownUntil = 0;

  // Fire any pending scheduled shots (Whitesnake Improvisation 3-burst rhythm).
  if (w.pendingShots && w.pendingShots.length) {
    const keep: typeof w.pendingShots = [];
    for (const s of w.pendingShots) {
      if (w.time >= s.fireAt) {
        w.projectiles.push({
          id: w.nextId++,
          pos: { x: s.pos.x + s.dir.x * 16, y: s.pos.y + s.dir.y * 16 },
          vel: { x: s.dir.x * s.speed, y: s.dir.y * s.speed },
          radius: s.radius,
          damage: s.damage,
          color: s.color,
          ownerKind: "player",
          pierce: false,
          hitSet: new Set(),
          expireAt: w.time + s.range / s.speed,
          knockbackAmount: s.knockback,
          textGlyph: s.textGlyph,
          sourceStandId: w.standId,
          sourceAbilityKey: s.sourceAbilityKey,
        });
        spawnParticles(w, { x: s.pos.x + s.dir.x * 12, y: s.pos.y + s.dir.y * 12 }, "#ffffff", 4, { speedMin: 40, speedMax: 120, life: 0.18 });
      } else {
        keep.push(s);
      }
    }
  ...
  // Activated prime-number heal (Whitesnake / C-Moon): press Prime Heal, then it says
  // the first 9 primes and ends cleanly after 23.
  if (pl.alive && (w.standId === "whitesnake" || w.standId === "c_moon") && w.primeHealUntil && w.time < w.primeHealUntil) {
    const PRIMES = [2, 3, 5, 7, 11, 13, 17, 19, 23];
    if ((w.primeIndex ?? 0) < PRIMES.length && (w.primeTickAt ?? 0) <= w.time) {
      w.primeTickAt = w.time + 1.6;
      const idx = w.primeIndex ?? 0;
      w.primeIndex = (w.primeIndex ?? 0) + 1;
      if (pl.hp < pl.maxHp) healPlayer(w, 2.5, w.standId === "c_moon" ? "#d6c5ff" : "#bff0c8");
      w.banners.push({
        id: w.nextId++, text: String(PRIMES[idx]),
        color: w.standId === "c_moon" ? "#d6c5ff" : "#bff0c8",
        expireAt: w.time + 1.0,
      } as never);
      if (idx === PRIMES.length - 1) {
        w.primeHealUntil = w.time + 0.15;
        w.bannerText = "Prime heal complete";
        w.bannerUntil = w.time + 1.0;
      }
    }
  } else if (w.primeHealUntil && w.time >= w.primeHealUntil) {
    w.primeHealUntil = 0;
    w.primeTickAt = 0;
  }
  ...
  if (!timeStopped) for (const e of w.npcs) {
    if (!e.alive) {
      ...
      e.vel.x *= 0.85;
      e.vel.y *= 0.85;
        if (!(e.cmoonFloatUntil && w.time < e.cmoonFloatUntil)) tryMove(e, e.vel.x * dt, e.vel.y * dt, w.props);
      continue;
    }
    ...
    if (e.kind === "enemy" && e.provoked && pl.alive && (dist2(e.pos, pl.pos) < ENEMY_AGGRO * ENEMY_AGGRO || (w.puppet.active && dist2(e.pos, w.puppet.pos) < ENEMY_AGGRO * ENEMY_AGGRO) || (w.hangedManActive && dist2(e.pos, w.hangedMan.pos) < ENEMY_AGGRO * ENEMY_AGGRO))) {
      ...
      // Standoff: stop advancing once we're inside our preferred melee distance so
      // we don't end up clipping into the target. Only close the last bit if very far away.
      const dToTarget = dist(e.pos, targetPos);
      const standoff = ENEMY_ATTACK_RANGE - 4;
      if (dToTarget > standoff) {
        tryMove(e, dir.x * ENEMY_SPEED * slowMul * dt, dir.y * ENEMY_SPEED * slowMul * dt, w.props);
      }
      e.facing = dir;
      if (dToTarget < ENEMY_ATTACK_RANGE && (!e.nextAttackAt || w.time >= e.nextAttackAt)) {
        ...
        // Stronger pushback (was 40) so player gets shoved when struck.
        else damageEntity(w, pl, dmg, { dir, amount: 110 });
      }
    } else if (e.kind !== "enemy" && pl.alive && (w.combatNoiseUntil ?? 0) > w.time && dist2(e.pos, pl.pos) < 180 * 180) {
      // Neutral NPC near active combat — bolt the other way.
      const away = norm({ x: e.pos.x - pl.pos.x, y: e.pos.y - pl.pos.y });
      tryMove(e, away.x * NPC_SPEED * 1.8 * dt, away.y * NPC_SPEED * 1.8 * dt, w.props);
      e.facing = away;
      e.wanderTarget = { x: e.pos.x + away.x * 200, y: e.pos.y + away.y * 200 };
      e.wanderUntil = w.time + 1.2;
    } else {
      // wander
  ...
  for (let i = 0; i < all.length; i++) {
    const a = all[i];
    if (!a.alive || (a.cmoonFloatUntil && w.time < a.cmoonFloatUntil)) continue;
    for (let j = i + 1; j < all.length; j++) {
      const b = all[j];
      if (!b.alive || (b.cmoonFloatUntil && w.time < b.cmoonFloatUntil)) continue;
  ...
  for (const pr of w.projectiles) {
    ...
    for (const e of w.npcs) {
      ...
      if (dist2(e.pos, pr.pos) < (pr.radius + e.radius) ** 2) {
        const hitDir = norm({ x: pr.vel.x, y: pr.vel.y });
        damageEntity(w, e, pr.damage, pr.knockbackAmount ? { dir: hitDir, amount: pr.knockbackAmount } : undefined);
        pr.hitSet.add(e.id);
        if (pr.stunSeconds) e.stunUntil = Math.max(e.stunUntil, w.time + pr.stunSeconds);
        if (pr.applyElectro) e.electroUntil = Math.max(e.electroUntil ?? 0, w.time + pr.applyElectro);
  ...
  if (w.nextArrowAt <= 0) {
    ...
    // Green W.I.P. Arrow: ~1/4 the rate of normal arrows.
    if (Math.random() < 0.25) trySpawnItem(w, "green_arrow");
    // Green Baby: very rare.
    if (Math.random() < 0.04) trySpawnItem(w, "green_baby");
  ...
  if (w.curseStrikes.length) {
    ...
    for (const c of w.curseStrikes) {
      ...
      // Area variant (Lightning Pillar): vertical cyan/yellow column at targetPos.
      if (c.targetPos && c.kind === "lightning_pillar") {
        const tp = c.targetPos;
        const skyAbove = { x: tp.x, y: tp.y - 260 };
        spawnVfx(w, { kind: "lightning_bolt", pos: skyAbove, to: { ...tp }, color: "#cffaff", life: 0.35 });
        spawnVfx(w, { kind: "lightning_bolt", pos: { x: tp.x - 4, y: tp.y - 260 }, to: { x: tp.x - 2, y: tp.y }, color: c.color, life: 0.35 });
        spawnVfx(w, { kind: "explosion_ring", pos: { ...tp }, radius: c.radius ?? 28, color: c.color, life: 0.45 });
        spawnParticles(w, tp, c.color, 20, { speedMin: 80, speedMax: 260, life: 0.55 });
        const r2 = (c.radius ?? 28) ** 2;
        for (const e of w.npcs) {
          if (!e.alive) continue;
          if (dist2(e.pos, tp) < r2) {
            damageEntity(w, e, c.dmg);
            if (c.stun) e.stunUntil = Math.max(e.stunUntil, w.time + c.stun);
          }
        }
        w.craters?.push?.({ pos: { ...tp }, radius: (c.radius ?? 28) * 0.6, expireAt: w.time + 10, bornAt: w.time } as never);
        w.shake = Math.max(w.shake, 6);
        continue;
      }
      const t = w.npcs.find((e) => e.id === c.targetId);
      if (!t || !t.alive) continue;
      if (c.kind === "cmoon_drop") {
        t.cmoonFloatUntil = 0;
        spawnVfx(w, { kind: "shockwave", pos: { ...t.pos }, radius: 24, color: c.color, life: 0.42 });
        spawnParticles(w, { x: t.pos.x, y: t.pos.y + 6 }, "#b7a5ff", 18, { shape: "square", gravity: 90, speedMin: 50, speedMax: 170, life: 0.55 });
...
export function tryPickupItems(w: World): { arrows: number; discs: number } {
  ...
  for (const it of w.items) {
    if (dist2(it.pos, w.player.pos) < (PICKUP_RADIUS + w.player.radius) ** 2) {
      ...
      else if (it.kind === "green_arrow") { w.greenArrowCount = (w.greenArrowCount ?? 0) + 1; play("pickupArrow"); showToast(w, "Picked up Green W.I.P. Arrow"); }
      else if (it.kind === "green_baby") { w.greenBabyCount = (w.greenBabyCount ?? 0) + 1; play("pickupArrow"); showToast(w, "Picked up Green Baby"); }
    } else remain.push(it);
  }
  w.items = remain;
...
function resetStandRuntime(w: World) {
  ...
  w.primeTickAt = 0;
  w.primeIndex = 0;
  w.primeHealCooldownUntil = 0;
  w.cmoonGravityUntil = 0;
  w.cmoonGravityAffected = [];
  for (const e of w.npcs) e.cmoonFloatUntil = 0;
  // Empty all per-stand bars on a stand swap so the new stand starts fresh.
  w.sptwRage = 0;
  w.rage = 0;
  w.whiteAlbumBar = 100;
  w.pendingShots = [];
...
function markStandUnlocked(w: World, id: StandId) {
  if (id === "none") return;
  w.standsUnlocked = w.standsUnlocked ?? [];
  if (!w.standsUnlocked.includes(id)) w.standsUnlocked.push(id);
}
...
export function useGreenArrow(w: World): boolean {
  if ((w.greenArrowCount ?? 0) <= 0) return false;
  if (w.standId !== "none") { showToast(w, "Use a DISC to drop your stand first"); return false; }
  w.greenArrowCount = (w.greenArrowCount ?? 0) - 1;
  resetStandRuntime(w);
  w.standId = "whitesnake";
  markStandUnlocked(w, "whitesnake");
  w.shitVariant = false;
  w.standActive = false;
  w.bannerText = "Got Stand: Whitesnake — tap Stand to summon";
  w.bannerUntil = w.time + 3;
  play("rollStand");
  return true;
}

export function useGreenBaby(w: World): boolean {
  if ((w.greenBabyCount ?? 0) <= 0) return false;
  if (w.standId !== "whitesnake") { showToast(w, "Need Whitesnake equipped"); return false; }
  w.greenBabyCount = (w.greenBabyCount ?? 0) - 1;
  resetStandRuntime(w);
  w.standId = "c_moon";
  markStandUnlocked(w, "c_moon");
  w.shitVariant = false;
  w.standActive = false;
  w.bannerText = "C-MOON awakens";
  w.bannerUntil = w.time + 3;
  play("rollStand");
  return true;
}
...
export function useBluePebble(w: World): boolean {
  ...
  markStandUnlocked(w, "moon_rabbit");
  w.shitVariant = false;
  w.standActive = false;
...
export function render(ctx: CanvasRenderingContext2D, w: World) {
  ...
  for (const it of w.items) {
    ...
    if (it.kind === "arrow" || it.kind === "requiem_arrow") {
    ...
    } else if (it.kind === "green_arrow") {
      const pulse = 0.4 + 0.4 * Math.sin(w.time * 4);
      ctx.fillStyle = `rgba(80,255,140,${0.45 * pulse})`;
      ctx.beginPath(); ctx.arc(cx, cy, 12, 0, Math.PI * 2); ctx.fill();
      ctx.save();
      ctx.translate(cx, cy); ctx.scale(SC, SC); ctx.rotate(-Math.PI / 4);
      ctx.fillStyle = "#2a3a22"; ctx.fillRect(-8, -1, 13, 2);
      ctx.fillStyle = "#5fe88a";
      ctx.beginPath(); ctx.moveTo(5, -5); ctx.lineTo(10, 0); ctx.lineTo(5, 5); ctx.closePath(); ctx.fill();
      ctx.fillStyle = "#bdf7c8";
      ctx.beginPath(); ctx.moveTo(6, -2); ctx.lineTo(9, 0); ctx.lineTo(6, 2); ctx.closePath(); ctx.fill();
      ctx.fillStyle = "#3e8a3a"; ctx.fillRect(-10, -3, 3, 6);
      ctx.restore();
    } else if (it.kind === "green_baby") {
      const pulse = 0.4 + 0.4 * Math.sin(w.time * 3.5);
      ctx.fillStyle = `rgba(90,230,120,${0.4 * pulse})`;
      ctx.beginPath(); ctx.arc(cx, cy, 13, 0, Math.PI * 2); ctx.fill();
      ctx.save();
      ctx.translate(cx, cy); ctx.scale(SC, SC);
      // body
      ctx.fillStyle = "#7ed089";
      ...
      ctx.fillRect(-2, -4, 1.4, 1.4); ctx.fillRect(1, -4, 1.4, 1.4);
      // gold cheek tinge
      ctx.fillStyle = "rgba(245,211,107,0.6)";
      ctx.fillRect(-3, -2, 1.2, 1); ctx.fillRect(2, -2, 1.2, 1);
      ctx.restore();
    } else {
      ctx.save();
      ctx.translate(cx, cy);
  ...
  for (const pr of w.projectiles) {
    ...
    if (pr.textGlyph === "WS_BULLET") {
      const ang = Math.atan2(pr.vel.y, pr.vel.x);
      ctx.save();
      ctx.translate(pr.pos.x, pr.pos.y);
      ctx.rotate(ang);
      ctx.fillStyle = "rgba(230,235,245,0.28)";
      ctx.fillRect(-10, -2, 12, 4);
      ctx.fillStyle = pr.color;
      ctx.beginPath(); ctx.ellipse(0, 0, 6, 2.4, 0, 0, Math.PI * 2); ctx.fill();
      ctx.fillStyle = "#ffffff";
      ctx.fillRect(2, -1, 2, 1);
      ctx.restore();
      continue;
    }
    if (pr.textGlyph && ["ゴゴゴ", "ドドド", "ピピピ", "ズキューン", "BLEED"].includes(pr.textGlyph)) {
      ctx.save();
      ctx.translate(pr.pos.x, pr.pos.y);
      ctx.rotate(Math.sin(w.time * 8) * 0.18);
      ctx.font = "bold 13px serif";
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.fillStyle = "rgba(0,0,0,0.65)";
      ctx.fillText(pr.textGlyph === "BLEED" ? "ゴゴゴ" : pr.textGlyph, 1, 1);
      ctx.fillStyle = pr.color;
      ctx.fillText(pr.textGlyph === "BLEED" ? "ゴゴゴ" : pr.textGlyph, 0, 0);
      ctx.restore();
      continue;
    }
    ctx.fillStyle = pr.color;
  ...
  for (const z of w.zones) {
    ...
    if (z.glyph) {
      ...
      ctx.rotate(-0.18 + Math.sin(w.time * 3) * 0.08);
      ctx.font = "bold 18px serif";
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.fillStyle = "rgba(0,0,0,0.65)";
      ctx.fillText(z.glyph, 1, 1);
      ctx.fillStyle = hexToRgba(z.color, 0.95);
      ctx.fillText(z.glyph, 0, 0);
      ctx.restore();
    }
  }
...
function drawPlayer(ctx: CanvasRenderingContext2D, w: World) {
  const pl = w.player;
  const cmoonFloating = w.standId === "c_moon" && !!w.cmoonGravityUntil && w.time < w.cmoonGravityUntil;
  if (cmoonFloating) {
    const started = w.cmoonGravityStartedAt ?? w.time;
    const lift = Math.min(26, (w.time - started) * 9) + Math.sin(w.time * 5) * 2;
    ctx.save();
  ...
  if (standVisible) drawStandAura(ctx, w, pl.pos, STANDS[w.standId].color);
  ...
  if (wearingWhiteAlbum) {
    ...
    ctx.fillRect(pl.pos.x - 5, pl.pos.y - 10, 10, 9);
    const fx = pl.facing.x, fy = pl.facing.y;
    const sptwEyes = w.standId === "sptw";
    ctx.fillStyle = sptwEyes ? "#5fe8ff" : "#222";
    ...
    if (sptwEyes) {
      // small cyan trail off the player's eyes
      const t = w.time * 6;
      for (let i = 0; i < 4; i++) {
        const ph = (t + i * 0.7) % 2;
        const a = Math.max(0, 1 - ph / 2);
      ctx.fillStyle = `rgba(95,232,255,${0.42 * a})`;
      ctx.fillRect(pl.pos.x - 3, pl.pos.y - 7 - ph * 3, 2, 1);
      ctx.fillStyle = `rgba(95,232,255,${0.42 * a})`;
      ctx.fillRect(pl.pos.x + 1, pl.pos.y - 7 - ph * 3, 2, 1);
      }
    }
  }
  ...
  if (cmoonFloating) ctx.restore();
}

function drawStandAura(ctx: CanvasRenderingContext2D, w: World, pos: Vec2, color: string) {
  for (let i = 0; i < 3; i++) {
    const pulse = ((w.time * (0.7 + i * 0.18) + i * 0.33) % 1);
    const r = 18 + pulse * 18 + i * 4;
    ctx.strokeStyle = hexToRgba(color, (1 - pulse) * (0.34 - i * 0.06));
    ctx.lineWidth = 2 - i * 0.35;
    ctx.beginPath(); ctx.ellipse(pos.x, pos.y + 3, r, r * 0.55, 0, 0, Math.PI * 2); ctx.stroke();
  }
  const glow = ctx.createRadialGradient(pos.x, pos.y, 3, pos.x, pos.y, 30);
  ...
  ctx.textAlign = "center";
  for (let i = 0; i < 3; i++) {
    const a = w.time * 1.1 + i * 2.1;
    ctx.fillStyle = hexToRgba(color, 0.15 + 0.08 * Math.sin(w.time * 3 + i));
    ctx.fillText("ゴ", pos.x + Math.cos(a) * 20, pos.y - 20 - ((w.time * 10 + i * 9) % 24));
  }
}
...
function drawStand(ctx: CanvasRenderingContext2D, w: World, pos: Vec2) {
  ...
  else if (id === "whitesnake") drawWhitesnake(ctx, w, pos);
  else if (id === "c_moon") drawCMoon(ctx, w, pos);
}

function drawWhitesnake(ctx: CanvasRenderingContext2D, w: World, pos: Vec2) {
  const punching = w.time < w.standPunchUntil || w.time < w.standAimUntil;
  const bob = Math.sin(w.time * 4.5) * 0.8;
  drawStandAura(ctx, w, { x: pos.x, y: pos.y + bob }, "#e8e8ee");
  ctx.fillStyle = "#dfe2e8";
  ctx.fillRect(pos.x - 5, pos.y - 2 + bob, 10, 13);
  ctx.fillStyle = "#8f98a8";
  ctx.fillRect(pos.x - 5, pos.y + 2 + bob, 10, 2);
  ctx.fillStyle = "#eef1f5";
  ctx.fillRect(pos.x - 4, pos.y - 11 + bob, 8, 9);
  ctx.fillStyle = "#7b8490";
  ctx.fillRect(pos.x - 4, pos.y - 9 + bob, 8, 1);
  ctx.fillStyle = "#14161a";
  ctx.fillRect(pos.x - 3, pos.y - 7 + bob, 2, 2);
  ctx.fillRect(pos.x + 1, pos.y - 7 + bob, 2, 2);
  ctx.fillStyle = "#b8c0cc";
  ctx.fillRect(pos.x - 2, pos.y - 4 + bob, 4, 1);
  const d = w.standPunchDir;
  const reach = punching ? 8 : 0;
  ctx.fillStyle = "#eef1f5";
  ctx.fillRect(pos.x - 9 + d.x * reach, pos.y + bob, 4, 6);
  ctx.fillRect(pos.x + 5 + d.x * reach, pos.y + bob, 4, 6);
  if (punching) {
    ctx.fillStyle = "#ffffff";
    ctx.fillRect(pos.x + d.x * 14, pos.y + d.y * 8 + bob, 2, 2);
  }
}

function drawCMoon(ctx: CanvasRenderingContext2D, w: World, pos: Vec2) {
  const punching = w.time < w.standPunchUntil || w.time < w.standAimUntil;
  const bob = Math.sin(w.time * 3.2) * 1.0;
  drawStandAura(ctx, w, { x: pos.x, y: pos.y + bob }, "#cfd2d8");
  ctx.fillStyle = "#cfd2d8";
  ctx.fillRect(pos.x - 6, pos.y - 2 + bob, 12, 13);
  ctx.fillStyle = "#7a6aa8";
  ctx.fillRect(pos.x - 6, pos.y + 1 + bob, 12, 2);
  ctx.fillStyle = "#e9edf0";
  ctx.beginPath(); ctx.ellipse(pos.x, pos.y - 8 + bob, 5, 6, 0, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = "#101218";
  ctx.fillRect(pos.x - 3, pos.y - 8 + bob, 2, 2);
  ctx.fillRect(pos.x + 1, pos.y - 8 + bob, 2, 2);
  ctx.strokeStyle = "#a06bff";
  ctx.lineWidth = 1;
  ctx.beginPath(); ctx.arc(pos.x, pos.y - 8 + bob, 7, -0.3, Math.PI + 0.3); ctx.stroke();
  const d = w.standPunchDir;
  const reach = punching ? 9 : 0;
  ctx.fillStyle = "#e9edf0";
  ctx.fillRect(pos.x - 10 + d.x * reach, pos.y + bob, 4, 6);
  ctx.fillRect(pos.x + 6 + d.x * reach, pos.y + bob, 4, 6);
}
...
function drawSptw(ctx: CanvasRenderingContext2D, w: World, pos: Vec2) {
  ...
  // stand eyes stay white; cyan eye trail belongs to the player.
  ctx.fillStyle = "#fff";
...
function drawNpc(ctx: CanvasRenderingContext2D, w: World, e: Entity) {
  const floating = !!e.cmoonFloatUntil && w.time < e.cmoonFloatUntil;
  if (floating) {
    const start = e.cmoonFloatStart ?? w.time;
    const lift = Math.min(34, (w.time - start) * 10) + Math.sin(w.time * 6 + e.id) * 3;
    ctx.save();
    ctx.translate(e.pos.x, e.pos.y - lift);
    ctx.rotate(Math.PI + Math.sin(w.time * 4 + e.id) * 0.18);
...
export function activatePrimeHeal(w: World): boolean {
  if (w.standId !== "whitesnake" && w.standId !== "c_moon") return false;
  if (!w.player.alive) return false;
  if (w.primeHealUntil && w.time < w.primeHealUntil) {
    showToast(w, "Prime heal already active");
    return false;
  }
  if (w.primeHealCooldownUntil && w.time < w.primeHealCooldownUntil) {
    showToast(w, `Prime heal ${Math.ceil(w.primeHealCooldownUntil - w.time)}s`);
    return false;
  }
  w.primeIndex = 0;
  w.primeTickAt = w.time;
  w.primeHealUntil = w.time + 14.7;
  w.primeHealCooldownUntil = w.time + 24;
  const color = w.standId === "c_moon" ? "#d6c5ff" : "#bff0c8";
  w.bannerText = "Prime heal";
  w.bannerUntil = w.time + 1.0;
  spawnVfx(w, { kind: "shockwave", pos: { ...w.player.pos }, radius: 38, color, life: 0.45 });
  spawnParticles(w, w.player.pos, color, 22, { shape: "spark", gravity: -80, speedMin: 20, speedMax: 120, life: 0.8 });
  play("standSummon");
  return true;
}
...
export function applySave(w: World, s: SaveData): { arrows: number; discs: number } {
  ...
  w.requiemArrowCount = s.requiemArrows ?? 0;
  w.bluePebbleCount = s.bluePebbles ?? 0;
  w.strangeHatCount = s.strangeHats ?? 0;
  w.tonthCopyCount = s.tonthCopies ?? 0;
  w.greenArrowCount = s.greenArrows ?? 0;
  w.greenBabyCount = s.greenBabies ?? 0;
